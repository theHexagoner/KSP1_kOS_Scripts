...

{
	LOCAL this IS LEXICON(
		"find_launch_window", find_launch_window@,
		"fill_rsvp", fill_rsvp@
	).

	LOCAL FUNCTION Fill_RSVP {
		PARAMETER that IS LEXICON().
		that:ADD("find_launch_window", find_launch_window@).
		SET rsvp TO that.
		RETURN that.
	}
	...
	local function iterated_local_search {
		...
		until x > latest_departure {
			...
			local departure_time is candidate:pos:x.
			local arrival_time is candidate:pos:x + candidate:pos:y.
