...

{
	...
	LOCAL FUNCTION Fill_RSVP {
		PARAMETER rsvp IS LEXICON().
		rsvp:ADD("line_search", line_search@).
		rsvp:ADD("grid_search", grid_search@).
		RETURN rsvp.
	}
	...
	local function line_search {
		...
		local pos is v(x, 0, 0).
		local minimum is cost(pos).
	
		return coordinate_descent(dimensions, cost, pos, minimum, step_size, step_threshold).
	}
	...
	local function coordinate_descent {
		parameter dimensions, cost, pos, minimum, step_size, step_threshold.
	
		local next_position is pos.
		local direction is "none".
	
		local function test {
			parameter test_direction.
	
			local test_position is pos + step_size * test_direction.
		...
		until step_size < step_threshold {
			...
			if direction = "none" {
				...
				set pos to next_position.
			}
		}
	
		return lex("pos", pos, "minimum", minimum).
	}
